...
import com.sijienet.card.game.Card;
import com.sijienet.card.game.Common;
 ...
 */
public class GameSenceActivity extends Activity {

    public static final String tag="GameSenceActivity";
    ...
    private List<Card> dizhuList = new Vector<Card>();//地主牌
    private int[] flag = new int[3];// 判断当前是否要牌
    private Context context;
    private Bitmap dzImg;
    private Bitmap bgImg;
    private Bitmap rearBitmap;
    private Handler handler;
    private boolean isRuns;//结束循环胜利控制
    ...
    //抢地主
    private void qiangDiZhu(boolean isQiang) {
        if (isQiang)
        {
            Common.dizhuFlag=1;
        }else {
    ...
    private void initCardArr() {
        ...
        for (int i = 1; i <= 4; i++) {// 四种花色
            for (int j = 3; j <= 15; j++) {// 15张牌
                ...
                cardArr[count].width=bitmap.getWidth();
                cardArr[count].height=bitmap.getHeight();
        ...
        //放入大小王
        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.a5_16);
        cardArr[52] =new Card();
        cardArr[52].clicked=false;
        cardArr[52].rear=false;
        cardArr[52].bitmap=bitmap;
        cardArr[52].name="a5_16";
        cardArr[52].width=bitmap.getWidth();
        cardArr[52].height=bitmap.getHeight();
        Bitmap bitmap2 = BitmapFactory.decodeResource(context.getResources(), R.drawable.a5_17);
        cardArr[53] =new Card();
        cardArr[53].clicked=false;
        cardArr[53].rear=false;
        cardArr[53].bitmap=bitmap;
        cardArr[53].name="a5_17";
        cardArr[53].width=bitmap2.getWidth();
        cardArr[53].height=bitmap2.getHeight();

        //地主图标
        dzImg = BitmapFactory.decodeResource(context.getResources(), R.drawable.image0);

        //背景图片
        bgImg = BitmapFactory.decodeResource(context.getResources(), R.drawable.bg);

        //纸牌背面
        rearBitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.cardbg1);
    }
}

...
public class Common {
    ...
    public static List<Card>[] cards;//所有牌合集

    // 得到电脑最佳选牌  获取最好的牌
    public static List<Card> getBestAI(List<Card> curList, List<Card> oppo) {
        try {
            List<Card> list = new Vector<Card>(curList);//可以出的牌综合
            Model model = new Model();//所有组合合集
            Model modelSingle = new Model();//独立牌合集
            ...
            Common.getSingle(list, model);//所有牌型可能性查出到model
            // 去除model里面独立牌型
                        ...
                        for (int l = 0, len4 = model.a123.size(); l <= len4; l++)
                            for (int m = 0, len5 = model.a112233.size(); m <= len5; m++)
                                for (int n = 0, len6 = model.a111222.size(); n <= len6; n++) {
                                    List<Card> newlist = new Vector<Card>(list);//剩下的所有可以出的牌
                                    // 我承认这个循环有点长..相信你的CPU 参数一 去出独立牌型的实体牌 第二 取出独立牌型的所有可能牌型
                                    bestModel = Common.getBestModel(newlist, model,
                                            new int[]{i, j, k, l, m, n});//从关联牌型中选择出可以出最好牌型
                                    // 加上独立的牌 还原所有可以出的牌型
            ...
            // 被动出牌
            if (showCardslList == null || showCardslList.size() == 0)//没有好的牌
                return null;
    ...
    public static List<Card> getMyBestCards(List<Card> current, List<Card> oppo) {
        ...
        Log.i("bins", Common.jugdeType(list).toString() + ",");
    ...
    public static void setOrder(List<Card> list) {
        Collections.sort(list, new Comparator<Card>() {// 重新排序接口
            @Override
    ...
    public static void rePosition(MyView view, List<Card> list, int flag) {
        if (flag == 1) {// 我自己
            for (int i = 0, len = list.size(); i < len; i++) {
                ...
                if (card.clicked)
                    y -= card.height / 3;
                card.setLocation(view.screen_width / 2 - (len / 2 - i)
                        * card.width * 2 / 3, y);
            }
        }
        if (flag == 0) {// 左边
            for (int i = 0, len = list.size(); i < len; i++) {
                Card card = list.get(i);
                card.setLocation(card.width / 2, card.height / 2 + i
                        * card.height / 7);
            }
        }
        if (flag == 2) {// 右边玩家
            for (int i = 0, len = list.size(); i < len; i++) {
                Card card = list.get(i);
                card.setLocation(view.screen_width - 3 * card.width / 2,
                        card.height / 2 + i * card.height / 7);
            }
        }
    }
    ...
    public static void getTwo(List<Card> list, Model model) {
        ...
        // 连续2张相同
        for (int i = 0, len = list.size(); i < len; i++) {
            if (i + 1 < len && Common.getValue(list.get(i)) == Common.getValue(list.get(i + 1))) {//判断是否是对子
    ...
    public static Model getBestModel(List<Card> list2, Model oldModel, int[] n) {
        // a4 a3 a2 a123 a112233 a111222 顺序固定 权重排行  一个个牌型选出 例如 先选炸弹 再选3带  再选对子 以此类推
        Model temp = new Model();
        // 处理炸弹
        for (int i = 0; i < n[0]; i++) {
            if (Common.isExists(list2, oldModel.a4.get(i))) { //从不是独立牌型中检查牌型是否存在
                temp.a4.add(oldModel.a4.get(i));//进入候选组
                list2.removeAll(Common.getCardsByName(list2, oldModel.a4.get(i)));//移除可以出去的牌组
            }
        }
        // 3带
        for (int i = 0; i < n[1]; i++) {
    ...
    public static void checkModel(List<Card> list, Model model1,
        ...
        for (int i = 0, len = model1.a2.size(); i < len; i++) {
            ...
            if (flag == 0) {
                modelSingle.a2.add(model1.a2.get(i));//添加计入单牌候选组
                list.removeAll(Common.getCardsByName(list, model1.a2.get(i)));//从可以出牌的组里面移除
            }
        }
        model1.a2.removeAll(modelSingle.a2);//从所有可能牌型中取出
    ...
    public static Boolean isLessFive() {
        if (!Common.isFriend() && cards[Common.oppoerFlag].size() <= 5) {
            return true;
        }
        return false;
    }
    ...
    public static Boolean isSingleOpper() {
        if (!Common.isFriend() && cards[Common.oppoerFlag].size() == 1)
            return true;
        else
            return false;
    }
    ...
    public static int getBestDizhuFlag() {
        ...
        for (int i = 0, len = cards[0].size(); i < len; i++) {
            if (Common.getValue(cards[0].get(i)) > 14)
                count1++;
        }
        for (int i = 0, len =cards[2].size(); i < len; i++) {
            if (Common.getValue(cards[2].get(i)) > 14)
                count2++;
        }
        if (count1 > count2)
            return 0;
...
//接口
interface callBack {
    void message(int what, Object obj);
}
